...
import torch
from torch.special import erf, Tensor
import torch.nn.functional as F
from torch import nn, tensor, linspace, is_tensor
from torch.nn import Module
from einx import subtract, divide, Callable
...
def HLGaussLoss(min_value,max_value,num_bins,sigma = None,sigma_to_bin_ratio = None,eps = 1e-10,clamp_to_range = False,min_max_value_on_bin_center = False):
    ...
    loss_fn = HLGaussLossFromSupport(support,sigma = sigma,sigma_to_bin_ratio = sigma_to_bin_ratio,eps = eps,clamp_to_range = clamp_to_range)
    return loss_fn

class HLGaussLossFromSupport(Module):
    def __init__(self,support: list[float] | Tensor,sigma = None,sigma_to_bin_ratio = None,eps = 1e-10,clamp_to_range = False):
        super().__init__()
        self.eps = eps
        ...
        sigma = default(sigma, sigma_to_bin_ratio * mean_bin_size) # default sigma to ratio of 2.
        self.sigma = sigma
        assert self.sigma > 0.
